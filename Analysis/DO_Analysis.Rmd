...
recently, by the Presumpscot Regional Land Trust (PRLT). The data were delivered
to CBEP by staff at PRLT in March of 2020, incorporating data through 2019.  PRW
began submitting data to DEP in 2009, so the data reviewed here only goes back
that far.

The goal of this analysis was to underpin graphics for the 2020 State of the Bay
Report.  However, because results at each monitoring site differs, the
analyses provide general background. Each site really needs to be evaluated on
its own terms.
...
Several values are apparent outliers in the source data. These are data that
have unlikely or unreasonable values, but for which we could not confirm a data
...
focusing on a "surface water only" data set simplifies the analysis.
```{r}
...
percentage, it is effectively a ratio of random variables, and so might
...
example, look at all relevant model diagnostics, and do not search for
alternative models that may provide better estimates of standard errors and 
nominal p values.  That may be important because we know both DO and Percent
Saturation figures are likely to be somewhat  skewed, with significant 
kurtosis. The justification is that we cannot look at results across all
monitoring sites, and probably should look at each monitoring site individually,
but we do not have the space in SoCB to do so effectively.  The only summaries
...
is low, and teh aalysis tends to hide site to site and year to year variation.
...
anova(fin_mod_2)
```
So, in both cases, the best model (by AIC) again includes both year and year by
site interaction terms, and fitting a separate value for each year provides a
...
presumpscot_data %>%
  ...
  geom_line()
```
Average water temperatures are not especially closely correlated with annual 
...
The thresholds, however, partially reflect Maine water quality standards.
